...
// PolarProductID is backend-only and never exposed in API responses.
type Pricing struct {
...
func Init(cfg PricingConfig) {
	if cfg.PolarProductIDBoostPlus != "" {
		options[0].Pricing.PolarProductID = cfg.PolarProductIDBoostPlus
	}
	if cfg.BoostPlusVND > 0 {
		options[0].Pricing.VND = cfg.BoostPlusVND
	}
	if cfg.PolarProductIDBoostPro != "" {
		options[1].Pricing.PolarProductID = cfg.PolarProductIDBoostPro
	}
