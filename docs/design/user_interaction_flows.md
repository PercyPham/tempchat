...
1. Client fetches `GET /api/rooms/:roomId` (to get `memberCount` and `maxParticipants`) and `GET /api/boost-options` in parallel.
2. **If any boost option has `maxParticipants > room.maxParticipants`:** Show a full-screen **"Room Is Full"** view with the group name and a list of applicable boost options as purchase cards.
...
3. **Floating Status Pill**: A dynamic pill floats at the top of the chat area:
   ...
   - **Interaction**: Tapping the pill expands it to show boost purchase options (fetched from `GET /api/boost-options`), each displayed as a **Boost Option Card**. Both options are always shown — stacking is always useful for extending expiry even if the participant cap is already high.
...
- **Trigger**: Tapping the **Floating Status Pill** or the "Boost Room" button in the **Group Detail** overlay.
- **Flow**: Bottom Sheet shows boost options (fetched from `GET /api/boost-options`) → user selects and pays (SePay / Paddle) → payment webhook confirms → server runs atomic boost script → `room:boosted` broadcast → pill timer and chat feed update for all members.
...
9. **Component: Boost Option Card** — Displays boost name, what it adds (expiry + cap), and price. Fetched from `GET /api/boost-options`. Reused in both the in-room Bottom Sheet and the Room Full screen.
