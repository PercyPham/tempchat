...
export function RoomDetailDrawer({ open, onClose, roomId, roomInfo, session, onBoost, memberNames }: Props) {
  ...
  const footer = (
    <button
      ...
      style={{ background: "var(--tc-leave-btn-bg)", border: "1px solid var(--tc-leave-btn-border)" }}
    >
      ...
      <div className="flex flex-col gap-6">
        ...
        <div>
          ...
          <button
            ...
            style={{
              background: "var(--tc-boost-action-bg)",
              border: "1px solid var(--tc-boost-action-border)",
              color: "#F59E0B",
            }}
          >
            ...
            <div className="flex flex-col items-start">
              ...
              {isFull && <span className="text-[11px] text-warm-white/35">Raise the limit to let more people in</span>}
            </div>
          </button>
        </div>
            ...
            {[...roomInfo.members]
              ...
              .map((member: PlainMember) => {
                ...
                return (
                  <div
                    key={member.uid}
                    className={`flex items-center gap-3 py-2 px-3 rounded-xl transition-colors ${hasLeft ? "opacity-40" : "hover:bg-warm-white/4"}`}
                  >
                    <div
                      className="h-8 w-8 rounded-xl flex items-center justify-center shrink-0 font-display font-bold text-sm uppercase"
                      style={
                        hasLeft
                          ? {
                              background: "var(--tc-member-left-bg)",
                              border: "1px solid var(--tc-member-left-border)",
                              color: "var(--tc-member-left-color)",
                            }
                          : {
                    ...
                    <span
                      className={`text-sm flex-1 ${hasLeft ? "text-warm-white/50 line-through decoration-warm-white/20" : "text-warm-white"}`}
                    >
                      {base}
...
function SectionLabel({ children }: { children: React.ReactNode }) {
  return <p className="text-[10px] font-semibold text-warm-white/25 uppercase tracking-[0.15em] mb-3">{children}</p>;
}
