...
import { genAsymmetricKeyPair, deriveAES256FromPrivate, encrypt } from "./crypto";
...
describe("AES-GCM ciphertext lengths (for backend limit sizing)", () => {
  ...
  beforeAll(async () => {
    ...
    key = await deriveAES256FromPrivate(privateKey);
  });
  ...
  for (const c of cases) {
    it(`${c.label} → ${b64Length(c.utf8Bytes)} chars`, async () => {
      const cipher = await encrypt(c.str, key);
